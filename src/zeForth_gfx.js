...
const src = objs_src.concat(`
  ...
  : zgfx_make_SlantVert
  # ( src run rise -- objref )
  .dhw zobj_HERE @   # ( src run rise objref )
  .dhw >R            # ( src run rise ) R:( objref )
  .dhw LIT_1 DUP     # ( src run rise 1 1 ) R:( objref )
  .dhw zobj_makeHDR  # ( src run rise hdr ) R:( objref )
  .dhw zobj_,        # ( src run rise ) R:( objref )
  .dhw (LIT)
  .dhw zgfx_(SlantVert)
  .dhw zobj_,        # ( src run rise ) R:( objref )
  .dhw LIT_3         # ( src run rise 3 ) R:( objref )
  .dhw zobj_allot    # ( src run rise ) R:( objref )
  .dhw R@            # ( src run rise  objref ) R:( objref )
  .dhw LIT_1         # ( src run rise objref 0 ) R:( objref )
  .dhw zobj_dat!     # ( src run ) R:( objref )
  .dhw R@            # ( src run objref ) R:( objref )
  .dhw LIT_0         # ( src run objref 0 ) R:( objref )
